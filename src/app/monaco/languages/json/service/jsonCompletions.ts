...
/**
 * BAS:
 * https://github.com/Microsoft/vscode-json-languageservice/blob/master/src/jsonContributions.ts
 */
...
export class JSONCompletion {
	...
	public doComplete(document: TextDocument, position: Position, doc: Parser.JSONDocument): Thenable<CompletionList> {
		...
		let offset = document.getOffsetAt(position);
		...
		if (node && (node.type === 'string' || node.type === 'number' || node.type === 'boolean' || node.type === 'null')) {
            const start = document.getPositionAt(node.start);
            const end = document.getPositionAt(node.end);
			overwriteRange = new Range(start.lineNumber,start.column,end.lineNumber,end.column);
			filterText = document.getValue().substring(node.start, offset);
		} else {
            const start = document.getPositionAt(offset - currentWord.length);
            const end = position;
			overwriteRange = new Range(start.lineNumber,start.column,end.lineNumber,end.column);
			filterText = document.getValue().substring(offset - currentWord.length, offset);
		}
		...
		return schemaService.getSchemaForResource(document.filePath).then((schema) => {
			...
			if (node) {
				...
				if (node.type === 'string') {
					...
					if (stringNode.isKey) {
						...
						currentKey = document.getValue().substring(node.start + 1, node.end - 1);
						if (node.parent) {
	...
	private getTextForMatchingNode(node: Parser.ASTNode, document: TextDocument): string {
		switch (node.type) {
			...
			case 'object':
				return '{}';
			default:
				let content = document.getValue().substr(node.start, node.end - node.start);
				return content;
		}
	}
