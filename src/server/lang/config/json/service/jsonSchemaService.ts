 ...
 *
 * The original uses `request-lite` but we ship with schemas.
 * We just use the raw-loader to load the files
 *
 */
/*---------------------------------------------------------------------------------------------
...
// TODO: json
// request-lite
const getErrorStatusDescription = (num: number) => num.toString();
type XHROptions = {}
type XHRResponse = {
    responseText: string;
    status: number;
}
const request = (options: XHROptions): Thenable<XHRResponse> => {
    return null;
}
...
export class JSONSchemaService implements IJSONSchemaService {
    ...
    private contextService: IWorkspaceContextService = { toResource: () => null };
	private callOnDispose: Function[];
    private telemetryService: ITelemetryService = { log: () => null };
